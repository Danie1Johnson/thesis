...
\begin{mylem}
\label{lem:I}
Let the Building Game intermediates $[x^j]$ and $[x^k]$ be connected in the combinatorial configuration space. Given $x^j \in [x^j]$, $x^k \in [x^k]$ and $f \in F$ such that $x^k = x^j \cup \left\{f\right\}$, the number of orbits $|F_{jk}/G_{x^j}| = |F_{kj}/G_{x^k}|$.
\end{mylem}
\begin{proof}
--NEED TO PROVVVVE
\end{proof}
