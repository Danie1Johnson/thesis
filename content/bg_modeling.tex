...
Let $G$ be a group acting on a set $X$, the \textbf{orbit} of an element $x \in X$ is the subset $G.x \doteq \left\{g.x : g \in G \right\}$ of $X$.
\end{mydef}

We also use the shorthand notation $[x]$ to refer to the orbit $G.x$. 
...
---explanation of polyhedral group
