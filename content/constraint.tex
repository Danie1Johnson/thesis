...
\begin{align}
z = \begin{bmatrix} v_1 \\ v_2 \\ \vdots \\ v_{n/3} \end{bmatrix} \in \mathbbm{R}^n
\end{align}
...
Using the notion of a constraint space, we can define the space of different geometric configurations that a given intermediate can take. Since we are treating each intermediate as collection of rigid polygons attached to each other along hinged edges, there are two basic constraints we must enforce. 

First, we must ensure that the each individual face is rigit and has the correct geometric shape. 
