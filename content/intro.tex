...
We have enumerated the combinatorial configuration space for all Platonic, Archimedean, and Catalan solids up to 30 faces. This constitutes the most comprehensive enumeration of the Building Game to date. 
...
Each Building Game intermediate is defined by its combinatorial structure, but by treating it as a linkage its geometric

By concatenating the 3-dimensional vertex locations, each geometric configuration can be specified as a point in an $n$-dimensional space. Since each geometric configuration must satify certain combinatorial and geometric properties, we enforce these conditions by using a set of algebraic constraint equations. The solution set of these constraint equations specifies an algebraic variety in the $n$-dimensional ambient space and the dimension of this variety give information about the mobility of the intermediate. To examine the geometric configuration space more thoroughly, we consider a reflected Brownian motion on the variety. Boundaries here are added to preclude the geometric configuration from intersecting itself when embedded in three dimensions.  
